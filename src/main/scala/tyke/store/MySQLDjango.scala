  ...
  def djangoBackingStore(transactor: Transactor[IO]) = new BackingUserStore[IO, String, User] {
        ...
        case algo :: iterations :: salt :: hash :: _ =>
          ...
          val calculatedHash =
            new String(Base64.getEncoder.encode(
              PBKDF2(pw.value.getBytes, salt.getBytes, iterations = Integer.parseInt(iterations), cryptoAlgo = algoMap(algo))))
          hash == calculatedHash
    ...
    private def doGet(id: String, pw: Option[Password[Source.Entered]]): OptionT[IO, User] = {
      ...
      where up.user_id=u.id and ug.id = aug.group_id and aug.user_id = u.id and u.username=$id
        """
      val ret = q.query[(Option[String], Option[String], Option[String], Option[Int], Option[String])].option.transact(transactor).map {
        case Some((Some(name), Some(storedPassword), Some(role), Some(is_staff), Some(groups))) => Some(UserTypes.UserAndPassword(
          User(
            username = name,

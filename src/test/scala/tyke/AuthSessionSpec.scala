...
class AuthSessionSpec extends org.specs2.mutable.Specification with BeforeEach with Http4sClientDsl[IO] {
  ...
  private val sessionStore = Dummy.dummySessionStore[IO, User, UserSession](UserSession, timeout = 1.second, now = virtualNow)
  private val serviceMaker = new AuthWithStore(conf=defaultConfig.server, userStore = userStore, sessionStore = sessionStore)
  ...
  "AuthWithSessions" >> {
    "happy path" >> {
      client.fetch(POST(Uri.uri("/login"), UserCredentials(user1.user.username, user1.password.value).asJson)).status mustEqual Status.Ok
    ...
    "valid login" >> {
      val resp = client.fetch(POST(Uri.uri("/login"), UserCredentials(user1.user.username, user1.password.value).asJson))
    ...
    "unauthorized access" >> {
      client.fetch(validateUri).status mustEqual Status.Ok
      client.fetch(POST(Uri.uri("/logout_user"), JustUsername(user2.user.username).asJson)).status mustEqual Status.Forbidden
    }
    "logout user" >> {
      val adminClient = Helpers.MicroClient(service)
      adminClient.fetch(POST(Uri.uri("/login"), UserCredentials(staffUser.user.username, staffUser.password.value).asJson)).status mustEqual Status.Ok
      adminClient.fetch(POST(Uri.uri("/logout_user"), JustUsername(user1.user.username).asJson)).status mustEqual Status.Ok
      client.fetch(validateUri).status mustEqual Status.Forbidden
    }
    "escalate" >> {
      // TODO: improve test
      val adminClient = Helpers.MicroClient(service)
      adminClient.fetch(POST(Uri.uri("/login"), UserCredentials(staffUser.user.username, staffUser.password.value).asJson)).status mustEqual Status.Ok
      adminClient.fetch(POST(Uri.uri("/escalate"), JustUsername(staffUser2.user.username).asJson)).status mustEqual Status.Forbidden
      adminClient.fetch(POST(Uri.uri("/logout_user"), JustUsername(staffUser2.user.username).asJson)).status mustEqual Status.Forbidden
      adminClient.fetch(POST(Uri.uri("/escalate"), JustUsername(user1.user.username).asJson)).status mustEqual Status.Ok
      adminClient.fetch(POST(Uri.uri("/logout_user"), JustUsername(user1.user.username).asJson)).status mustEqual Status.Forbidden
      adminClient.fetch(POST(Uri.uri("/escalate"), JustUsername(user1.user.username).asJson)).status mustEqual Status.Forbidden
    }
    "session timeout" >> {
      client.fetch(POST(Uri.uri("/login"), UserCredentials(user1.user.username, user1.password.value).asJson)).status mustEqual Status.Ok
      client.fetch(validateUri).status mustEqual Status.Ok
